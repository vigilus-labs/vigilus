...
from vigilus.db.models import Operator, PermissionLevel, Provider, Tool, TrustMode
...
async def test_seed_preserves_user_edits_to_builtin_operators(db_session: AsyncSession):
    ...
    op = (
        (
            await db_session.execute(
                select(Operator).where(Operator.is_builtin.is_(True))
            )  # noqa: E712
        )
        .scalars()
        .first()
    ...
    new_trust = TrustMode.lenient if op.trust_mode != TrustMode.lenient else TrustMode.strict
