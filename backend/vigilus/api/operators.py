...
from vigilus.db.models import Operator, Tool, OperatorTool, Provider
from vigilus.db.seed import RESEARCH_TOOL_NAMES, VIGILUS_PRINCIPAL_NAME
from vigilus.schemas.operator import OperatorCreate, OperatorUpdate, OperatorResponse
...
async def get_operator(operator_id: str, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Operator).options(selectinload(Operator.operator_tools)).where(Operator.id == operator_id))
...
async def create_operator(data: OperatorCreate, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(select(Operator).options(selectinload(Operator.operator_tools)).where(Operator.id == op.id))
    return _to_response(result.scalar_one())

@router.patch("/{operator_id}", response_model=OperatorResponse)
async def update_operator(operator_id: str, data: OperatorUpdate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Operator).options(selectinload(Operator.operator_tools)).where(Operator.id == operator_id))
    op = result.scalar_one_or_none()
    if not op:
        raise HTTPException(status_code=404, detail="Operator not found")
        
    if data.name is not None: op.name = data.name
    if data.description is not None: op.description = data.description
    if data.system_prompt is not None: op.system_prompt = data.system_prompt
    if data.soul is not None: op.soul = data.soul
    if data.provider_id is not None: op.provider_id = data.provider_id
    ...
    if "model" in data.model_fields_set: op.model = data.model
    if data.permission_level is not None: op.permission_level = data.permission_level
    if data.trust_mode is not None: op.trust_mode = data.trust_mode
    if data.working_dir is not None: op.working_dir = data.working_dir
    if data.enabled is not None: op.enabled = data.enabled
    if data.icon is not None: op.icon = data.icon
...
async def test_operator(operator_id: str, data: TestRunRequest, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Operator).options(selectinload(Operator.operator_tools).selectinload(OperatorTool.tool), selectinload(Operator.provider)).where(Operator.id == operator_id))
    op = result.scalar_one_or_none()
    if not op:
        raise HTTPException(status_code=404, detail="Operator not found")
    ...
    try:
        ...
        return {
            "ok": True,
            "messages": [{"role": m.role, "content": str(m.content), "name": m.name} for m in final_msgs],
