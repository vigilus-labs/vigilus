...
down_revision: Union[str, None] = '9a2c4e70b1d8'
...
def upgrade() -> None:
    ...
    if not _has_table("scans"):
        op.create_table(
            ...
            sa.Column('source', sa.Enum('nmap', 'wazuh', 'manual', 'custom', name='scopesource'),
                      nullable=False, server_default='nmap'),
    ...
    if not _has_index('discovered_hosts', 'ix_discovered_hosts_matched_server_id'):
        op.create_index('ix_discovered_hosts_matched_server_id', 'discovered_hosts', ['matched_server_id'])
    ...
    if not _has_table("discovered_services"):
        op.create_table(
            ...
            sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
            sa.ForeignKeyConstraint(['discovered_host_id'], ['discovered_hosts.id'], ondelete='CASCADE'),
            sa.PrimaryKeyConstraint('id'),
    ...
    if not _has_table("findings"):
        op.create_table(
            ...
            sa.Column('id', sa.String(length=36), nullable=False),
            sa.Column('source', sa.Enum('nmap', 'wazuh', 'manual', 'custom', name='scopesource'),
                      nullable=False, server_default='wazuh'),
            sa.Column('kind', sa.Enum('alert', 'vulnerability', 'fim', 'exposure', name='findingkind'),
                      nullable=False),
            sa.Column('severity', sa.Enum('info', 'low', 'medium', 'high', 'critical', name='findingseverity'),
